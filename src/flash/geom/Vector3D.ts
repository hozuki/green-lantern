...
export class Vector3D implements ICloneable<Vector3D>, ICopyable<Vector3D> {
    ...
    nearEquals(toCompare:Vector3D, tolerance:number, allFour:boolean = false):boolean {
        return _util.isValueBetweenNotEquals(this.x, toCompare.x - tolerance, toCompare.x + tolerance) &&
            _util.isValueBetweenNotEquals(this.y, toCompare.y - tolerance, toCompare.y + tolerance) &&
            _util.isValueBetweenNotEquals(this.z, toCompare.z - tolerance, toCompare.z + tolerance) && !(
                allFour && !_util.isValueBetweenNotEquals(this.w, toCompare.w - tolerance, toCompare.w + tolerance)
            );
    }
    ...
    toString():string {
        return _util.formatString("[x={0}, y={1}, z={2}, w={3}]", this.x, this.y, this.z, this.w);
    }
